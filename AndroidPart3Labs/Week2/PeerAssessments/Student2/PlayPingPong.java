...
public class PlayPingPong implements Runnable {
    ...
    // @@ TODO - you fill in here.
  		...
  		CyclicBarrier myBarrier = new CyclicBarrier(2);
    ...
    class PingPongThread extends HandlerThread implements Handler.Callback {
        ...
        protected void onLooperPrepared() {
        	...
        	pingHandler= new Handler(this.getLooper(),this);
        		...
        		else 
        		{
        			...
        			pongHandler= new Handler(this.getLooper(),this);
            ...
            if (this.mMyType == PingPong.PING) {
            	...
            	Message myMessage = pingHandler.obtainMessage();
            	myMessage.setTarget(pingHandler);
            	myMessage.obj=pongHandler;
                pingHandler.sendMessage(myMessage);      	
            	}
        ...
        public boolean handleMessage(Message reqMsg) {
        	...
        	if (mIterationsCompleted < mMaxIterations) {
            		...
            		mOutputStrategy.print(System.getProperty("line.separator")+ mMyType.toString() + ": " + Integer.toString(mIterationsCompleted));
            ...
            myMsg.setTarget((Handler)reqMsg.obj);
            myMsg.obj = reqMsg.getTarget();
            ...
            myMsg.getTarget().sendMessage(myMsg);
            return true;
        }
    }
    ...
    public void run() {
        ...
        PingPongThread myPingThread = new PingPongThread(PingPong.PING);
        PingPongThread myPongThread = new PingPongThread(PingPong.PONG);
