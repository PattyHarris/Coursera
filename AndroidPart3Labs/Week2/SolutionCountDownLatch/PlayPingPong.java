...
public class PlayPingPong implements Runnable {
    ...
    /** 
     * PH: Define a CountDownLatch for coordinating the thread completions.
     */
    CountDownLatch mCountDownLatch = new CountDownLatch( mArrayHandlers.length );
    ...
    class PingPongThread extends HandlerThread implements Handler.Callback {
        ...
        /**
         * PH: CountDownLatch used for coordinating thread
         * completion.
         */
         ...
         */
        public PingPongThread(PingPong myType, CountDownLatch stopLatch) {
        ...
        public boolean handleMessage(Message reqMsg) {
            ...
            if (mIterationsCompleted < mMaxIterations) {
            	...
            	// thread to quit.  Decrement the CountDownLatch too...
                mStopLatch.countDown();

            	Log.i(TAG, "handleMessage() Time to quit.");
                reqHandler.getLooper().quit();
            ...
            // There should be a way to uses the CyclicBarrier here...
    ...
    public void run() {
        ...
        PingPongThread pingThread = new PingPongThread(PingPong.PING, mCountDownLatch);
        PingPongThread pongThread = new PingPongThread(PingPong.PONG, mCountDownLatch);
