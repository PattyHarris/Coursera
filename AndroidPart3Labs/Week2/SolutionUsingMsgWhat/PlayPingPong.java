    ...
    class PingPongThread extends HandlerThread implements Handler.Callback {
        ...
        protected void onLooperPrepared() {
            ...
            if (mMyType == PingPong.PING) {
            	...
            	pingMsg.what = PingPong.PING.ordinal();
        ...
        public boolean handleMessage(Message reqMsg) {
    		...
    		Log.i(TAG, "handleMessage() reqMsg.what " + String.format("%d", reqMsg.what) );

        	Handler reqHandler =  mArrayHandlers[ reqMsg.what ];
        	int newWhat = PingPong.PING.ordinal();
        	
            if (mIterationsCompleted < mMaxIterations) {
            	
            	String output = "PONG";
            	
            	if ( reqMsg.what == PingPong.PING.ordinal() ) {
            		
            		output = "PING";
            		newWhat = PingPong.PONG.ordinal();
            	}
            	
            	mIterationsCompleted ++;
            	mOutputStrategy.print(output + String.format(" (%d)\n", mIterationsCompleted) );
            	...
            	// PH: We need to tell the looper to quit.
            	// reqHandler.getLooper().getThread().interrupt();
            ...
            // PH: New target is the message's object.  The "what"
            // contains the sender's type, but probably isn't needed here.
